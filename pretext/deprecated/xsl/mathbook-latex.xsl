...
<xsl:template name="latex-preamble">
    ...
    <!-- CLP styling -->
    <!--  ADR added colour definitions space here -->
    <xsl:text>%%&#xa;</xsl:text>
    <xsl:text>%% Define colours for styles&#xa;</xsl:text>
    <xsl:call-template name="style-default-colours" />
    <xsl:text>%%&#xa;</xsl:text>
    <!-- end of CLP styling -->
    ...
    <!-- CLP styling. ADR added the following  -->
    <!-- To allow footnote styling -->
    <xsl:call-template name="style-footnote-preamble" />
    <!-- To allow figure styling -->
    <xsl:call-template name="style-figure-preamble" />
    <!-- To allow sbs styling -->
    <xsl:call-template name="style-sbs-preamble" />
    <!-- To allow exercise styling -->
    <xsl:call-template name="style-exercise-preamble" />
    <!-- end of CLP styling -->
    ...
    <xsl:if test="$document-root//exercises//exercise[not(ancestor::exercisegroup)]|$document-root//worksheet//exercise[not(ancestor::exercisegroup)]|$document-root//reading-questions//exercise[not(ancestor::exercisegroup)]">
        ...
        <!-- CLP styling -->
        <!-- ADR added the following for marked exercises -->
        <xsl:text>\tcbset{ markeddivisionexercisestyle/.style={markedexstyle} }&#xa;</xsl:text>
        <xsl:text>\newtcolorbox{markeddivisionexercise}[4]</xsl:text>
        <xsl:text>{markeddivisionexercisestyle, before title={\hspace{-5ex}\makebox[5ex][l]{#1.}}, title={\notblank{#2}{#2\space}{}}, phantom={\hypertarget{#4}{}}, after={\notblank{#3}{\newline\rule{\workspacestrutwidth}{#3\textheight}\newline}{}}}&#xa;</xsl:text>
        <!-- end of CLP styling -->
    ...
    <xsl:if test="$document-root//exercisegroup[not(@cols)]">
        ...
        <!-- CLP styling -->
        <!-- ADR added the following for marked exercises -->
        <xsl:text>\tcbset{ markeddivisionexerciseegstyle/.style={markedexegstyle} }&#xa;</xsl:text>
        <xsl:text>\newtcolorbox{markeddivisionexerciseeg}[4]</xsl:text>
        <xsl:text>{markeddivisionexerciseegstyle, before title={\hspace{-5ex}\makebox[5ex][l]{#1.}}, title={\notblank{#2}{#2\space}{}}, phantom={\hypertarget{#4}{}}, after={\notblank{#3}{\newline\rule{\workspacestrutwidth}{#3\textheight}\newline}{}}}&#xa;</xsl:text>
        <!-- end of CLP styling -->
    </xsl:if>
    <!-- Division Solution, Exercise Group, Columnar -->
    <!-- Explicity unbreakable, to behave in multicolumn tcbraster -->
...
<!-- CLP styling: ADR added -->
<!-- for default style colours -->
...
<!-- CLP styling -->
<!--  Misc styling preamble things added by ADR -->
<!-- by default no footnote styling -->
<xsl:template name="style-footnote-preamble"/>
<!-- so default-styling of figure is only simple centering -->
<xsl:template name="style-figure-preamble"/> <!--no preamble required by defult  -->
<xsl:template name="style-figure-head"> <!-- before the figure -->
  <xsl:text>\begin{figure}&#xa;</xsl:text>
</xsl:template>
<xsl:template name="style-figure-before-caption">     <!-- before caption -->
  <xsl:text>\centering&#xa;</xsl:text>
</xsl:template>
<xsl:template name="style-figure-foot">     <!-- at end of figure -->
<xsl:text>\end{figure}&#xa;</xsl:text>
</xsl:template>
<!-- default-styling of sbs  -->
<xsl:template name="style-sbs-preamble"/> <!--no preamble required by defult  -->
<!-- default-styling of exercise  -->
<xsl:template name="style-exercise-preamble"/> <!--no preamble required by defult  -->
<!-- end of CLP styling -->
...
<xsl:template match="exercises//exercise|worksheet//exercise|reading-questions//exercise">
    ...
    <xsl:variable name="env-name">
      <!-- CLP styling.  -->
      <!--  Test for marked exercises here -->
        <xsl:if test="@purpose='RQS'">
...
<xsl:template match="figure">
    ...
    <!-- CLP styling -->
    <!-- figure style hacking here -->
    <xsl:call-template name="style-figure-head" /> <!-- before the figure -->
    <xsl:apply-templates select="*[not(self::caption)]"/>     <!-- body of figure -->
    <xsl:call-template name="style-figure-before-caption"/>     <!-- before caption -->
    <xsl:apply-templates select="caption" />     <!-- the caption -->
    <xsl:call-template name="style-figure-foot"/>     <!-- at end of figure -->
    <!-- end of CLP styling -->
</xsl:template>
...
<xsl:template match="sidebyside" mode="compose-panels">
    ...
    <!-- CLP styling -->
    <!-- side-by-side styling only if not part of figure or sbsgroup -->
    <xsl:if test="not(parent::sbsgroup or parent::figure)">
      <xsl:call-template name="style-sbs-head"/>
    </xsl:if>
    <!-- end of clp styling -->
    ...
    <xsl:if test="not(parent::sbsgroup or parent::figure)">
      <xsl:call-template name="style-sbs-foot"/>
    </xsl:if>
    <!-- end of clp styling -->
...
<!-- CLP styling -->
<!-- call sbsgroup head and foot when needed -->
<!-- Requires corresponding hacks in mathbook-common -->
<xsl:template name="sbsgroup-head">
        <xsl:call-template name="style-sbsgroup-head"/>
</xsl:template>
<xsl:template name="sbsgroup-foot">
        <xsl:call-template name="style-sbsgroup-foot"/>
</xsl:template>
<!-- end of CLP styling -->
