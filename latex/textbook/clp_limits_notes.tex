...
curve whose coordinates are very close to $P$. Now rather than picking some
...
If having $h$ as a variable rather than a number bothers you, start by
...
\end{align*}
but please use the notation in Definition~\ref{def informal onesided
limits} above.
...
back from limits and think about how we construct functions. To
...
\end{align*}
\end{itemize}
So the product of these two factors will become larger and larger (and
...
the limits of more complicated functions in terms of simpler ones. When limits of
...
We have seen that computing the limits of some functions --- polynomials and
...
\end{itemize}
So that breaks down the theorem statement by statement, but what does it actually
mean?
\begin{itemize}
...
$f(x)$ has to be very close to $L$. How close is ``very close''? Well
