...
\end{hint}
\begin{answer} False. Lemma~\ref*{lem dxn} \emph{does not apply}. See the solution.
\end{answer}
\begin{solution}
False: Lemma~\ref*{lem dxn} tells us that, for a constant $n$, $\ds\diff{}{x}\{x^n\}=nx^{n-1}$. Note that the base $x$ is the variable and the exponent $n$ is a constant. In the equation given in the question, the base $2$ is a constant, and the exponent $x$ is the variable: this is the opposite of the situation required by Lemma~\ref*{lem dxn}. So Lemma~\ref*{lem dxn} \emph{does not apply}.

We do not yet know how to differentiate $2^x$. We'll learn about it in Section~\ref*{sec exp func}. There we will see that the derivative is $(\log_e 2)\,2^x$ \emph{not} $x\, 2^{x-1}$. 
\end{solution}
